...
# starts from the one the environment-dump rules use above -- a word character or
# ``-`` before the verb means it is the tail of another token (``rsync``, ``vnc``,
# ``async``) -- and adds ``.``, ``:`` and ``=``, which glue the letters to a file
# name or an assignment rather than to a command (``dataset.nc -e 50``, a
# ``*.nc -exec`` find clause once its quotes are normalized, ``X=nc``). ``/``
...
_NETCAT_EXEC_PATTERN = r"(?<![\w.:=-])nc\s+-e"
