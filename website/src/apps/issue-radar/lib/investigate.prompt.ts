...
• Before deciding the nature, check the repository's decision record — in Kiro Crew that is \`docs/decisions/\`, one file per decision a person on the team made. Grep it for the label, control or behaviour the issue names. When the issue collides with an entry, it is not a bug and not needs-info: state the collision in the summary, use verdict \`decided\`, set next_action to "close as decided — see <entry path>", and suggest no code change. Only a maintainer writing a superseding entry changes that.
• Search the codebase for the relevant code / error messages / symbols. Decide the issue's nature — bug | feature | question | duplicate | needs-info | decided — find the likely root cause or the code area involved, and check for related or duplicate issues in this repo.
