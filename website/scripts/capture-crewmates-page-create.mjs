 ...
 *   08-roster-load-failed-<theme>  the roster's first read failed: the list
 *                       column says so with a retry, and the chat column
 *                       (>= md, no chat open) carries the same notice instead
 *                       of a blank pane or the empty hero
 *
...
async function newPage(theme, scene, viewport = { width: 1440, height: 900 }) {
  ...
  await page.route(u => new URL(u).pathname.startsWith('/api/'), route => {
    ...
    if (path === '/api/members') {
      ...
      if ((scene === 'roster-fail' || scene === 'unconfirmed') && rosterReads > 1) return route.fulfill({ status: 500, contentType: 'application/json', body: '{"error":"roster unavailable"}' })
...
for (const theme of ['dark', 'light']) {
  ...
  check(`05-post-create-error-${theme} add held`, await page.getByTestId('member-add').isDisabled(), 'header + disabled while a follow-up step is pending')
  check(`05-post-create-error-${theme} add title`, (await page.getByTestId('member-add').getAttribute('title')) === 'Refresh the list for Radar before adding another crewmate.', 'roster-kind hold names only the retry')
  await page.waitForTimeout(400)
...
for (const theme of ['dark', 'light']) {
  ...
  check(`08-roster-load-failed-${theme} list notice`, await listNotice.getByText('Could not load your crewmates.', { exact: false }).isVisible(), 'list column says the read failed')
  check(`08-roster-load-failed-${theme} column notice`, await columnNotice.isVisible(), 'chat column carries the notice at >= md')
